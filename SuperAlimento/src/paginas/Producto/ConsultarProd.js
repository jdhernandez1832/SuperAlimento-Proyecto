...
const ConsultarProd = () => {
    ...
    useEffect(() => {
      const fetchProducto = async () => {
        try {
          const response = await fetch('http://localhost:3001/api/producto/todos'); // Asegúrate de que estás llamando a la ruta correcta para obtener productos
          if (response.ok) {
                    ...
                    <div className="col-12">
                        <div className="card">
                                ...
                                <table ref={tableRef1} className="table table-hover text-nowrap">
                                    ...
                                    {productos.map((producto) => (
                                      producto.estado !== 'Desactivo' && (
                                        ...
                                        <td><Link to={`/ActualizarProdu/${producto.id_producto}`} className="btn btn-warning">Actualizar</Link></td>
                                        <td>
                                          <button 
                            ...
                            <div className="card-header">
                                ...
                                <Link to="#" className="btn btn-primary custom-button">Generar reporte productos próximos de caducidad</Link>
                            </div>
                        </div>
                    </div>
                </div>
